...
EAN13.prototype.cantkDraw = function(element) {
    if(this.bincode && element) {
        var size = 2;
        element.resize(size * 95, size * 50)
        var that = this;
        element.drawBgImage = function(canvas) {
            canvas.fillStyle = 'white';
            canvas.fillRect(0, 0, this.w, this.h);
            canvas.fillStyle = 'black';
            for(var _y = 0, _r = 0; _r < 50; _r++) {
                for(var _x = 0, _t = 0; _t < 95; _t++) {
                    that.bincode[_t] == 1 && canvas.fillRect(_x, _y, size, size)
                    _x += size;
                }
                _y += size;
            }
